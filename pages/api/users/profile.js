  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      try {
        ...
        const result = await client.query(query, [req.user.userId, ...values]);
        
        // Fetch the updated user data
        const updatedUserResult = await client.query('SELECT username, email FROM users WHERE id = $1', [req.user.userId]);
